...
function getSpekColor(intensity: number): string {
  if (intensity < 0.08) {
    const t = intensity / 0.08;
    return `rgb(0, 0, ${Math.floor(t * 80)})`;
  } else if (intensity < 0.18) {
    const t = (intensity - 0.08) / 0.1;
    return `rgb(${Math.floor(t * 50)}, ${Math.floor(t * 30)}, ${Math.floor(
      80 + t * 175
    )})`;
  } else if (intensity < 0.28) {
    const t = (intensity - 0.18) / 0.1;
    return `rgb(${Math.floor(50 + t * 150)}, ${Math.floor(30 - t * 30)}, ${Math.floor(
      255 - t * 55
    )})`;
...
function getTimeStep(duration: number): number {
